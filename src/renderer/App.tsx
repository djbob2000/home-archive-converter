...
const App = () => {
  ...
  const { settings, updateOutputSetting, updateDownscaleSetting, updateAdvancedSetting } = useProcessingSettings();
  const handlePostConversion = useCallback(() => {
    clearItems();
  }, [clearItems]);

  const { startConversion, cancelConversion, isProgressOpen, progress, statusText, percentage } = useConversion({
    inputItems: items,
    settings,
    ...
    <div className="flex h-full flex-col font-sans">
      ...
      <div className="flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto bg-slate-100">
          ...
          {activeTab === 'settings' && <SettingsTab settings={settings.advanced} onChange={updateAdvancedSetting} />}
